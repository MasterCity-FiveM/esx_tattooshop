...
function OpenShopMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'tattoo_shop', {
		...
		if data.current.value == 'del_tattoo' then
		--ESX.ShowNotification(data.current.value)
			ESX.TriggerServerCallback('esx_tattooshop:del', function(success)
				if success then
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			exports.pNotify:SendNotification({text = 'جهت دسترسی به منو لطفا E بزنید.', type = "info", timeout = 4000})
		end
